...
public class SecurityConfig {
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		// CSR 공격: 'client'과 'web server' 사이에 다른 submit이 들어옴
		// 해결방안: submit 할 때 토큰을 같이 주어야 함 (CSR 토큰정책) -> 안 쓸거임.
		authorizeHttpRequest.requestMatchers(new AntPathRequestMatcher("/**")).permitAll())
		.sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.ALWAYS))
		.csrf((csrf) -> csrf.ignoringRequestMatchers(new AntPathRequestMatcher("/**")))
		.formLogin((formLogin) -> formLogin.loginPage("/auth/login").defaultSuccessUrl("/main"))
